...
export const userResolvers = {
  Query: {
    ...
    publicProfile: async (_root: unknown, { accountName }: { accountName: string }, context: Context) => {
      ...
      return {
        ...
        stats: { minutesSynced: user.stats?.minutesSynced ?? 0, wordsSynced: user.stats?.wordsSynced ?? 0, karaokeLines: user.stats?.karaokeLines ?? 0, syncedLines: user.stats?.syncedLines ?? 0 },
  ...
  User: {
    ...
    stats:           (user: IUser) => ({ minutesSynced: user.stats?.minutesSynced ?? 0, wordsSynced: user.stats?.wordsSynced ?? 0, karaokeLines: user.stats?.karaokeLines ?? 0, syncedLines: user.stats?.syncedLines ?? 0 }),
